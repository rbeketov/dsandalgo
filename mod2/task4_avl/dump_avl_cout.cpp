...
class AVLTree {
 public:
    ...
    T* findKthElem(const size_t k) {
        ...
        // std::cout << "Кол-во элементов в руте" << cnt(root) << std::endl;
        // std::cout << "реальное кол-во" << items_count << std::endl;
        // std::cout << "Кол-во элементов слева" << cnt(root->left_) << std::endl;
        // std::cout << "Кол-во элементов справа" << cnt(root->right_) << std::endl;
 ...
 private:
    ...
    void rightFixCnt(Node* node) {
        // std::cout << "До поворота слева " << cnt(node->left_) << std::endl;
        // std::cout << "До поворота справа " << cnt(node->right_) << std::endl;
    ...
    Node* balance(Node* node) {
        ...
        if (bf == 2) {
            if (bfacotr(node->right_) < 0) {
                ...
                // std::cout << "малое правое" << std::endl;
                node->right_ = rotateRight(node->right_);
                // std::cout << "после поворота слева " << cnt(node->right_->left_) << std::endl;
                // std::cout << "после поворота справа " << cnt(node->right_->right_) << std::endl;
            }
            leftFixCnt(node);
            // std::cout << "малое левое" << std::endl;
            Node* tmp = rotateLeft(node);
            // std::cout << "после поворота слева " << cnt(tmp->left_) << std::endl;
            // std::cout << "после поворота справа " << cnt(tmp->right_) << std::endl;
            // std::cout << "после поворота рут " << cnt(tmp) << std::endl;
            return tmp;
        }
        ...
        if (bf == -2) {
            if (bfacotr(node->left_) > 0) {
                ...
                // std::cout << "после поворота слева " << cnt(node->left_->left_) << std::endl;
                // std::cout << "после поворота справа " << cnt(node->left_->right_) << std::endl;
            }
            rightFixCnt(node);
            // std::cout << "малое правое" << std::endl;
