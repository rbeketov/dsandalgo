...
void Encode(IInputStream& original, IOutputStream& compressed) {
    ...
    //std::cout << cnt_table.size() << std::endl;
    ...
    // for ( auto it : haffman::toBinary(1) ) {
    //     std::cout << (int)it;
    // }
    // std::cout << std::endl;
...
void preOrderCreateTable(Node* node, std::unordered_map<byte, std::vector<byte> >& code_table, std::vector<byte> curr_bytes) {
    ...
    if (!node->left && !node->right) {
        // if (curr_bytes.size() == 0) {
        //     curr_bytes.push_back(1);
        // }
